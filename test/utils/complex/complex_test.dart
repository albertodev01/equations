...
void main() {
  group("Testing constructors", () {
    ...
    test("Making sure that real numbers are properly converted into complex ones.", () {
    ...
    test("Making sure that Complex objects are properly built from Fraction objects", () {
      final fromFraction = Complex.fromFraction(Fraction(3, 5), Fraction(1, 4));
      expect(fromFraction.real, equals(1.5));
      expect(fromFraction.imaginary, equals(0.25));

      final fromRealFraction = Complex.fromRealFraction(Fraction(3, 5));
      expect(fromRealFraction.real, equals(1.5));
      expect(fromRealFraction.imaginary, isZero);

      final fromImaginaryFraction = Complex.fromImaginaryFraction(Fraction(1, 4));
      ...
      final fromMixedFraction = Complex.fromMixedFraction(
        MixedFraction.fromString("-2 5/4"), MixedFraction.fromString("1 3/5")
      );
      expect(fromMixedFraction.real, equals(-2.5));
      expect(fromMixedFraction.imaginary, equals(0.25));

      final fromRealMixedFraction = Complex.fromRealMixedFraction(
        MixedFraction.fromString("-2 5/4")
      );
      expect(fromRealMixedFraction.real, equals(1.5));
      ...
      expect(fromImaginaryMixedFraction.imaginary, equals(0.25));
    });
  ...
  group("Testing the API of the Complex class", () {
    test("Making sure that the conjugate changes the sign of the imaginary part", () {
    ...
    test("Making sure that the reciprocal is actually 1/(a + bi)", () {
      ...
      expect(Complex.fromImaginary(1).conjugate(), equals(Complex.fromImaginary(-1)));

      expect(() => Complex.zero().reciprocal(), throwsA(isA<ComplexException>()));
    });

    test("Making sure that modulus (or 'magnitude' or 'absolute' value) is correct", () {
      expect(Complex(3, 7).abs().toStringAsFixed(12), equals(math.sqrt(58).toStringAsFixed(12)));
    });
    ...
    test("Making sure that sine, cosine, tangent and cotangents work properly on Complex", () {
      final i = Complex.i();

      expect(i.sin().imaginary.toStringAsFixed(12), equals("1.175201193644"));
      expect(i.cos().imaginary.toStringAsFixed(12), equals("1.543080634815"));
  ...
  group("Testing complex numbers operators", () {
    ...
    test("Making sure that the difference between two complex numbers is correct", () {
      final value = Complex(3, -5) - Complex(-8, 13);
