...
void main() {
  group("Testing the behaviors of the SylvesterMatrix class.", () {
    ...
    test("Making sure that the list of coefficients is unmodifiable.", () {
      expect(() => matrix.coefficients[0] = Complex.zero(),
          throwsA(isA<UnsupportedError>()));
    });

    test("Making sure that the discriminant is properly computed.", () {
      expect(matrix.polynomialDiscriminant(), equals(Complex(17, 0)));
      expect(matrix.polynomialDiscriminant(optimize: false),
          equals(Complex(17, 0)));
    });

    test("Making sure that the determinant is properly computed.", () {
      expect(matrix.matrixDeterminant(), equals(-Complex(17, 0)));
    });

    test(
        ("Making sure that SylvesterMatrix instances can be properly compared."),
        () {
      final matrix2 = SylvesterMatrix(coefficients: [
        Complex.fromReal(1),
        Complex.fromReal(-7),
        Complex.fromReal(8),
      ]);
