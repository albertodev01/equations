...
void main() {
  group("Testing the 'Laguerre' class for polynomial roots finding", () {
    test("Making sure that a 'Laguerre' object is properly constructed", () {
      ...
      expect(equation.discriminant(), const Complex(1, 12));
      expect(
      ...
      expect(equation[2], equals(const Complex(-3, -2)));
      ...
      expect(equation.coefficient(0), equals(const Complex(-3, -2)));
      ...
      expect(
        solutions[1].real,
        const MoreOrLessEquals(-1.1748, precision: 1.0e-4),
      );
      expect(
        solutions[1].imaginary,
        const MoreOrLessEquals(2.7768, precision: 1.0e-4),
      );
      expect(
        solutions[0].real,
        const MoreOrLessEquals(1.1748, precision: 1.0e-4),
      ...
      expect(
        ...
        const MoreOrLessEquals(0.2232, precision: 1.0e-4),
      );

      // Evaluation
      final eval = equation.realEvaluateOn(2);
      expect(eval, equals(const Complex(3, 2)));
    });
    ...
    test(
      ...
      expect(laguerre[0], equals(const Complex.fromReal(1)));
      expect(laguerre[1], equals(const Complex.fromReal(2)));
      expect(laguerre[2], equals(const Complex.fromReal(3)));

      // There must be an exception is the first coeff. is zero
      expect(
        () => Laguerre.realEquation(coefficients: [0, 3, 6]),
    ...
    test(
      ...
      expect(
        () => Laguerre(coefficients: const [Complex.zero()]),
        throwsA(isA<AlgebraicException>()),
      );
    });
